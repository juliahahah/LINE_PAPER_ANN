...
測試每分鐘發送時，請暫時把執行函式改成 `sendTestReminder`，並選「分鐘計時器 → 每分鐘」。測試結束後必須改回 `sendDailyReminder`，避免重複洗群組。
...
| `設定報告人數 1` 或 `設定報告人數 2` | 管理員 | 將後續每場會議設定為一位或兩位報告人 |
...
提醒訊息也會提供「我要請假」、「完成報告」、「本次一人報告」和「本週不開會」按鈕；後兩者限管理員使用。「本次一人報告」只影響下一場會議，第二位保留在輪值順序中。
